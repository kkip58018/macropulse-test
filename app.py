...
import time
from db import supabase_auth, supabase_admin, save_session, load_session,get_session_file
from utils import sign_in, sign_up, sign_out 
...
<style>
    ...
    /* Hide default sidebar nav on all pages (even if not logged in) */
...
# ---------- Authentication functions (copy from original) ----------
...
if not st.session_state.authenticated:
    # Try to restore session from cookie/file if email exists
    if st.session_state.user_email:
        if load_session(st.session_state.user_email):
            st.rerun()
