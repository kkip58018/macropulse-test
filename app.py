...
from db import save_session, load_session,get_session_file
...
# Fixed sidebar – toggleable via custom button (smooth slide transition)
if st.session_state.get("authenticated", False):
    sidebar_visible = st.session_state.get("sidebar_visible", True)
    sidebar_width = "21rem" if sidebar_visible else "0px"
    st.markdown(
        f"""
    <style>
        /* Prevent main content from hiding under the Streamlit header */
        .block-container {{
            padding-top: 3.5rem !important;
        }}
        
        /* Make header background match dark theme */
        header[data-testid="stHeader"] {{
            background-color: #0b0f15 !important;
        }}
        
        /* Hide the collapse/expand toggle button completely */
        ...
        /* Keep sidebar content at fixed width to avoid reflow during collapse */
        [data-testid="stSidebarContent"] {{
            width: 21rem !important;
            min-width: 21rem !important;
        }}
        
        /* Main content padding slides smoothly with the sidebar */
...
def sign_in(email, password):
    try:
        ...
        if response.user:
            ...
            if not profile.data or not profile.data[0].get("approved", False):
            ...
            save_session(response.session, email)  # <-- add email argument
            return True, None
        else:
